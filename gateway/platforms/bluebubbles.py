...
class BlueBubblesAdapter(BasePlatformAdapter):
    ...
    async def _handle_webhook(self, request):
        ...
        # Normalize chat_id: BlueBubbles webhook payloads inconsistently omit
        # the top-level chatGuid, causing 1:1 DM sessions to sometimes resolve
        # to the full "any;-;+<phone>" GUID and sometimes to a bare "+<phone>"
        # chat_identifier for the SAME physical conversation. Anything keyed
        # on chat_id (active-agent persistence, dedup hashing) then silently
        # fails to match across these two representations. Canonicalize a
        # bare-phone identifier to the dominant GUID format for 1:1 chats.
        if session_chat_id and not is_group and session_chat_id.startswith("+") and ";" not in session_chat_id:
            session_chat_id = f"any;-;{session_chat_id}"
