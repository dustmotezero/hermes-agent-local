...
LAZY_DEPS: dict[str, tuple[str, ...]] = {
    ...
    "memory.hindsight": ("hindsight-all==0.10.0",),  # [FIX 2026-09-16] Was hindsight-client==0.6.1, a stale pin that silently downgraded the correct hindsight-all install on every daemon start (upstream issue #86992). local_embedded mode needs hindsight-all (provides HindsightEmbedded), not the plain REST-only hindsight-client.
