...
def _blocked_url_in_code(code: str) -> Optional[str]:
    ...
    [FIXED 2026-09-22] Was importing evaluate_url_safety, which does not
    exist in tools.browser_tool — every browser_exec call crashed with an
    ImportError before ever running. Switched to the real, existing
    tools.url_safety.is_safe_url (same SSRF/private-IP oracle browser_tool
    itself uses internally as _is_safe_url).
    """
