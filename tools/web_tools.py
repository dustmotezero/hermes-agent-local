...
def web_search_tool(query: str, limit: int = 5, timelimit: Optional[str] = None) -> str:
    """
    ...
    # [ADDED 2026-09-22] Auto-bias toward fresh results for time-sensitive
    # queries when the caller didn't explicitly set timelimit. Mechanical,
    # not prompt-dependent — a query containing "today"/"latest"/"current"/
    # "right now" is very likely asking for something recent, and ddgs
    # otherwise has no reason to prefer a fresh result over an aged,
    # highly-relevant-looking cached one (confirmed real tonight: stale
    # week-old results outranking current ones on identical queries).
    if timelimit is None:
        _q_lower = (query or "").lower()
        _temporal_markers = ("today", "latest", "current", "right now", "this week", "breaking")
        if any(marker in _q_lower for marker in _temporal_markers):
            timelimit = "d"

    # [ADDED 2026-09-22] Auto-inject site: for commonly-named sources.
    # Confirmed real tonight: a generic phrase like "Hacker News top stories
    # today" returns inconsistent, often-irrelevant or stale-mirror results
    # (Wikipedia, third-party readers, a mirror frozen since Sep 15) across
    # repeated calls -- not caching, just low relevance for unscoped queries.
    # Pairing an explicit site: filter with the phrase reliably surfaced
    # real, current, correctly-sourced results in direct testing. Mechanical,
    # not prompt-dependent -- extend _KNOWN_SOURCE_DOMAINS as more named
    # sources come up.
    _q_lower_src = (query or "").lower()
    if "site:" not in _q_lower_src:
        for _name, _domain in _KNOWN_SOURCE_DOMAINS.items():
            if _name in _q_lower_src and _domain not in _q_lower_src:
                query = f"{query} site:{_domain}"
                break
    ...
    try:
        ...
        if provider is None:
            ...
            if timelimit is not None:
                try:
                    ...
                    if "timelimit" in _inspect.signature(provider.search).parameters:
                        response_data = provider.search(query, limit, timelimit=timelimit)
                    else:
        ...
        if isinstance(response_data, dict) and response_data.get("success"):
            response_data["note"] = "Results are search-engine snippets ordered by relevance, not by the site's own ranking or recency. Do not describe result order as a front-page rank. Check any date shown before calling something current."
...
# [ADDED 2026-09-22] Known source-name -> domain map for auto site: injection.
# Extend as needed; keys are matched as lowercase substrings of the query.
...
WEB_SEARCH_SCHEMA = {
    "name": "web_search",
    "description": "Search the web for information. Returns up to 5 results by default with titles, URLs, and descriptions. The query is passed through to the configured backend, so operators such as site:domain, filetype:pdf, intitle:word, -term, and \"exact phrase\" may work when the backend supports them. When the person names a specific site or source (e.g. \"on Hacker News\", \"on the company blog\"), prefer results from that site's own domain over third-party aggregators, mirrors, or trackers, which can lag the real source by days. Check any date attached to a result against today's actual date before describing it as current, latest, or \"today's\" — a real, accurate result that is several days old is not the same as a current one, and should be labeled with its real date instead of presented as fresh.",
    "parameters": {
        "type": "object",
        "properties": {
            "query": {
            ...
            "timelimit": {
                "type": "string",
                "description": "Optional freshness filter: 'd' (past day), 'w' (past week), 'm' (past month), 'y' (past year). Use 'd' or 'w' for questions about today's/current/latest news or events to avoid stale cached results. Omit for general or historical queries.",
                "enum": ["d", "w", "m", "y"]
            }
        },
        "required": ["query"]
...
registry.register(
    ...
    handler=lambda args, **kw: web_search_tool(args.get("query", ""), limit=args.get("limit", 5), timelimit=args.get("timelimit")),
