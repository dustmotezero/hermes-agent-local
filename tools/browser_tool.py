...
BROWSER_HARNESS_TIMEOUT = 60

BROWSER_EXEC_SCHEMA = {
    "name": "browser_exec",
    "description": (
        "Execute Python code in a persistent real-browser session via "
        "browser-harness (real CDP control of local Chrome). Pre-imported "
        "helpers: new_tab(url), goto_url(url), page_info(), "
        "click_at_xy(x, y), type_text(text), js(code), cdp(method, ...), "
        "wait_for_load(), list_tabs(), ensure_real_tab(). The namespace "
        "persists across calls within one squad-router turn. Returns "
        "whatever the code prints — nothing is fabricated. First navigation "
        "must be new_tab(url), not goto_url(url). Prefer the accessibility "
        "tree (cdp('Accessibility.getFullAXTree')) to find elements rather "
        "than screenshots; use a screenshot only when layout or imagery "
    ...
    "parameters": {
        ...
        "properties": {
            "code": {
                "type": "string",
                "description": "Python code to execute in the browser-harness session",
            },
        },
        "required": ["code"],
    },
...
def _guard_browser_exec_code(code: str):
    """
    Best-effort safety scan of browser_exec code before it runs. Mirrors the
    checks browser_navigate always applied via the (now-removed)
    evaluate_url_safety(): no secrets, no cloud metadata endpoint, no
    private/internal targets (unless explicitly allowed), no
    policy-blocked hosts. Regex-based scan of the whole code string, not a
    sandboxed static analysis -- will not catch every obfuscation, but
    restores the protection level browser_navigate always had.
    [ADDED 2026-09-25] browser_exec shipped with no equivalent guard at all.
    """
    import re as _re
    import urllib.parse
    from agent.redact import _PREFIX_RE

    if _PREFIX_RE.search(code) or _PREFIX_RE.search(urllib.parse.unquote(code)):
        return {"error": "Blocked: code appears to contain an API key or token. Secrets must not be passed to browser_exec."}

    local = _is_local_backend()
    url_re = _re.compile(r"""https?://[^\s'"<>]+|(?<![\w.])(?:\d{1,3}\.){3}\d{1,3}(?::\d+)?(?![\w.])""")
    for url in url_re.findall(code):
        norm = _normalize_url_for_request(url) if url.startswith("http") else url
        if _is_always_blocked_url(norm):
            return {"error": f"Blocked: code targets a cloud metadata endpoint ({url})"}
        if norm.startswith("http"):
            sensitive_query_key = _sensitive_query_param_name(norm)
            if sensitive_query_key and not local:
                return {"error": (
                    f"Blocked: code contains a credential-like query parameter ({sensitive_query_key}). "
                    "Cloud browser backends are third-party readers; remove the sensitive query parameter "
                    "or use a local browser/CDP session.")}
        if not local and not _allow_private_urls() and not _is_safe_url(norm):
            return {"error": f"Blocked: code targets a private or internal address ({url})"}
        if norm.startswith("http"):
            blocked = check_website_access(norm)
            if blocked:
                return {"error": blocked["message"]}
    return None


def _handle_browser_exec(code: str):
    if not isinstance(code, str) or not code.strip():
        return tool_error("browser_exec failed: 'code' must be a non-empty string")
    _guard_result = _guard_browser_exec_code(code)
    if _guard_result is not None:
        return tool_error(_guard_result.get("error", "browser_exec failed: blocked by safety guard"))
    try:
        result = subprocess.run(
            ...
            capture_output=True,
            text=True,
            timeout=BROWSER_HARNESS_TIMEOUT,
        )
    except subprocess.TimeoutExpired:
        return tool_error(
            f"browser_exec failed: browser-harness did not respond within "
            f"{BROWSER_HARNESS_TIMEOUT}s. The Chrome daemon may be unresponsive "
            f"or a permission popup may be waiting for a user click."
        )
    except FileNotFoundError:
        return tool_error(
            "browser_exec failed: 'browser-harness' command not found on PATH."
        )
    except Exception as e:
        return tool_error(f"browser_exec failed: {e}")

    if result.returncode != 0:
        stderr_tail = (result.stderr or "")[-1000:]
        return tool_error(
            f"browser_exec failed (exit {result.returncode}): {stderr_tail or '(no stderr)'}"
        )
