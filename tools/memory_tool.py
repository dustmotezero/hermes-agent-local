...
class MemoryStore:
    ...
    def _render_block(self, target: str, entries: List[str]) -> str:
        ...
        separator = "═" * 46
        return f"{separator}\n{header}\n{separator}\n" + ENTRY_DELIMITER.join(f"[{i}] {e}" for i, e in enumerate(entries, 1))
...
def memory_tool(
    ...
    if operations and (action or old_text):
        return tool_error("Use EITHER operations OR action/old_text/content, never both. For one change send only action, target, old_text and content.", success=False)
    ...
    if action == "add":
        for _k, _e in enumerate(store._entries_for(target), 1):
            if _e[:25] == content.strip()[:25]:
                return tool_error("That entry already exists as [%d]. Use action replace with old_text %d and the full new text in content." % (_k, _k), success=False)
    if False:
        return tool_error("Content is required for 'add' action.", success=False)
    if action in ("replace", "remove") and old_text:
        old_text = re.sub(r"^\[\d+\]\s*", "", old_text.strip())
        if len(old_text) > 80:
            old_text = old_text[:40]
    if action in ("replace", "remove") and old_text and old_text.strip().isdigit():
        _ents = store._entries_for(target)
        _n = int(old_text.strip())
        if 1 <= _n <= len(_ents):
            old_text = _ents[_n - 1]
        else:
            return tool_error("No entry number %d. Entries: " % _n + " | ".join("[%d] %s" % (i + 1, e[:50]) for i, e in enumerate(_ents)), success=False)
    if action == "replace" and (not old_text or not content):
        ...
        return tool_error(f"replace needs two fields. old_text: a short phrase (under 8 words) copied from the entry to change. content: the complete new entry text. Missing: {missing}. Resend both.", success=False)
...
MEMORY_SCHEMA = {
    ...
    "description": (
        ...
        "HOW: make ONE change per call using ONLY these fields: action (add, replace or remove), target, old_text (a short unique phrase from the existing entry; needed for replace and remove) and content (the full new entry; needed for add and replace). For several changes, make several calls. Do not use operations. Entries are numbered in your memory block; for replace or remove you may set old_text to just the entry number, for example 7\n\n"
