...
# Profiles that map flat to the root HERMES_HOME rather than nesting under
# profiles/<name>. "default" is the built-in root profile. "diane" is
# grandfathered in: her real production footprint (kanban.db, SOUL.md,
# scripts/, cron/) lives directly at ~/.hermes, not nested, because
# start_gateway.py never routes through profile resolution at all.
FLAT_MAPPED_PROFILES = {"default", "diane"}


def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its HERMES_HOME directory."""
    canon = normalize_profile_name(name)
    if canon in FLAT_MAPPED_PROFILES:
