...
FIND_SCHEMA = {
    "name": "hindsight_find",
    "description": "Search long-term memory for editable facts. Returns a numbered list. Run this before hindsight_correct.",
    "parameters": {
        "type": "object",
...
CORRECT_SCHEMA = {
    "name": "hindsight_correct",
    "description": "Rewrite one stored fact. First run hindsight_find, then give the result number and the complete corrected fact. Use a full date in new_text, never today or yesterday.",
    "parameters": {
        "type": "object",
        "properties": {
            "number": {"type": "integer", "description": "Result number from hindsight_find."},
            "new_text": {"type": "string", "description": "The complete corrected fact."},
        },
...
class HindsightMemoryProvider(MemoryProvider):
    ...
    def get_tool_schemas(self) -> List[Dict[str, Any]]:
        if self._memory_mode == "context":
            return []
        return [RETAIN_SCHEMA, RECALL_SCHEMA, REFLECT_SCHEMA, FIND_SCHEMA, CORRECT_SCHEMA]

    def _hs_edit_tools(self, tool_name, args):
        import urllib.request, urllib.error, urllib.parse
        base = str(self._api_url).rstrip("/")
        bank = urllib.parse.quote(str(self._bank_id), safe="")
        ...
        try:
            if tool_name == "hindsight_find":
                q = (args.get("query") or "").strip()
                if not q:
                    return tool_error("Missing required parameter: query")
                res = _call("POST", "/v1/default/banks/%s/memories/recall" % bank, {"query": q})
                facts = [r for r in res.get("results", []) if r.get("type") in ("world", "experience")][:8]
                self._found_fact_ids = [r["id"] for r in facts]
                if not facts:
                    return json.dumps({"result": "No editable facts found. Try different words."})
                return json.dumps({"result": "\n".join("%d. %s" % (i, r["text"]) for i, r in enumerate(facts, 1))})
            ids = getattr(self, "_found_fact_ids", [])
            new_text = (args.get("new_text") or "").strip()
            try:
                n = int(args.get("number"))
            except Exception:
                return tool_error("number must be the result number from hindsight_find, for example 2.")
            if not new_text:
                return tool_error("new_text is required: the complete corrected fact.")
            if not (1 <= n <= len(ids)):
                return tool_error("No result %d. Run hindsight_find first; it returned %d results." % (n, len(ids)))
            out = _call("PATCH", "/v1/default/banks/%s/memories/%s" % (bank, ids[n - 1]), {"text": new_text})
            if (out.get("text") or "").strip() != new_text:
                return tool_error("Server did not confirm the new text. It reads: %s" % out.get("text"))
            return json.dumps({"result": "Verified. Fact %d now reads: %s" % (n, new_text)})
        except Exception as e:
            logger.warning("%s failed: %s", tool_name, e, exc_info=True)
            return tool_error("Hindsight edit failed: %s" % e)
    ...
    def handle_tool_call(self, tool_name: str, args: dict, **kwargs) -> str:
        ...
        elif tool_name in ("hindsight_find", "hindsight_correct"):
            return self._hs_edit_tools(tool_name, args)
