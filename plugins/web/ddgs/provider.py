...
def _run_ddgs_search(query: str, safe_limit: int, timelimit: Optional[str] = None) -> list[dict[str, Any]]:
    ...
    ``timelimit``: ddgs freshness filter — "d" (day), "w" (week), "m" (month),
    "y" (year), or None for unrestricted. [ADDED 2026-09-22] Queries about
    "today"/"latest"/"current" events were silently returning aged/cached
    results with no way to bias toward recency; this exposes ddgs's own
    freshness parameter instead of relying on prompt-text alone.
    """
    from ddgs import DDGS  # type: ignore

    results: list[dict[str, Any]] = []
    with DDGS(timeout=10) as client:
        for i, hit in enumerate(client.text(query, max_results=safe_limit, timelimit=timelimit)):
            if i >= safe_limit:
                break
...
def _run_ddgs_search_bounded(query: str, safe_limit: int, timelimit: Optional[str] = None) -> list[dict[str, Any]]:
    ...
    *its* GIL — it only polls a communicator thread and, on timeout/interrupt,
    ...
    request: dict[str, Any] = {"query": query, "safe_limit": safe_limit, "timelimit": timelimit}
    if _test_hook:
...
class DDGSWebSearchProvider(WebSearchProvider):
    ...
    def search(self, query: str, limit: int = 5, timelimit: Optional[str] = None) -> Dict[str, Any]:
        ...
        ``timelimit``: optional ddgs freshness filter ("d"/"w"/"m"/"y").
        """
        try:
        ...
        try:
            web_results = _run_ddgs_search_bounded(query, safe_limit, timelimit=timelimit)
        except TimeoutError:
            logger.warning(
