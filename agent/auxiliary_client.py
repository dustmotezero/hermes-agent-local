...
def _fixed_temperature_for_model(
    ...
    if _is_arcee_trinity_thinking(model):
        return 0.5
    if model and "qwen3" in model.lower() and "instruct" in model.lower() and "thinking" not in model.lower():
        logger.debug("Fixed temperature 0.3 for Qwen3 Instruct model %r (grounded/tool-use turns)", model)
        return 0.3
    return None
