...
def build_api_kwargs(agent, api_messages: list, tools_for_api: list | None = None) -> dict:
    ...
    try:
        ...
        logger.info("DEBUG temp calc: agent.model=%r base_url=%r -> _ft=%r omit=%r fixed=%r", agent.model, agent.base_url, _ft, _omit_temp, _fixed_temp)
    except Exception as _temp_exc:
        _omit_temp = False
        _fixed_temp = None
        logger.warning("DEBUG temp calc FAILED for agent.model=%r: %r", getattr(agent, "model", None), _temp_exc)
