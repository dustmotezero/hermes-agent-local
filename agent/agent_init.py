...
def init_agent(
    ...
    except Exception as _cfg_load_e:
        # [FIX 2026-09-16] Was a bare silent swallow -- any config load
        # failure here (including for standalone/squad-router-constructed
        # agents) meant memory, skills, and compression config all silently
        # fell back to {}, with zero visibility into why. Real root cause
        # of "DIANE iMessage conversations never reach Hindsight" traced
        # to exactly this swallow.
        import logging as _cfg_log
        _cfg_log.getLogger(__name__).error(
            "agent_init: config load failed, falling back to {} for memory/skills/compression: %s",
            _cfg_load_e, exc_info=True,
        )
        _agent_cfg = {}
    ...
    # DIANE patch: respect model.context_length in config.yaml as override.
    from agent.model_metadata import MINIMUM_CONTEXT_LENGTH
    from hermes_cli.config import load_config as _load_ctx_cfg
    _ctx = getattr(agent.context_compressor, "context_length", 0)
    _cfg_ctx = cfg_get(_load_ctx_cfg(), "model", "context_length", default=None)
    _effective_minimum = min(MINIMUM_CONTEXT_LENGTH, int(_cfg_ctx)) if _cfg_ctx else MINIMUM_CONTEXT_LENGTH
    ...
    if _ctx and _ctx < _effective_minimum and not _allow_lmstudio_explicit_below_floor:
        raise ValueError(
            f"Model {agent.model} has a context window of {_ctx:,} tokens, "
            f"which is below the minimum {_effective_minimum:,} required "
            f"by Hermes Agent.  Choose a model with at least "
            f"{_effective_minimum // 1000}K context.  If your server "
            f"reports a window smaller than the model's true window, set "
            f"model.context_length in config.yaml to the real value. "
            f"(Local override: PATCH-016, see PATCHES.md -- this deployment "
            f"intentionally allows values below the {MINIMUM_CONTEXT_LENGTH // 1000}K "
            f"upstream default.)"
        )
