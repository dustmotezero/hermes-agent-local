...
class ChatCompletionsTransport(ProviderTransport):
    ...
    def build_kwargs(
        ...
        **params,
        ...
        # Temperature — legacy path (see docstring: fixed_temperature/omit_temperature
        # are computed by _fixed_temperature_for_model() and passed in as params,
        # but were never being read here). [FIXED 2026-09-22]
        if params.get("omit_temperature"):
            pass  # Don't include temperature at all
        elif params.get("fixed_temperature") is not None:
            api_kwargs["temperature"] = params.get("fixed_temperature")
            logger.info("Fixed temperature %r applied for model %r (legacy path)", params.get("fixed_temperature"), model)
        else:
        ...
        if overrides:
            # R-060: cache_prompt is llama.cpp-specific — must go in extra_body,
            # not top-level kwargs (SDK rejects unknown top-level fields).
            _llama_keys = {"cache_prompt"}
            _llama_overrides = {k: v for k, v in overrides.items() if k in _llama_keys}
            _std_overrides = {k: v for k, v in overrides.items() if k not in _llama_keys}
            if _llama_overrides:
                _eb = api_kwargs.get("extra_body") or {}
                _eb.update(_llama_overrides)
                api_kwargs["extra_body"] = _eb
            if _std_overrides:
                api_kwargs.update(_std_overrides)
    ...
    def _build_kwargs_from_profile(self, profile, model, sanitized, tools, params):
        ...
        if profile.fixed_temperature is OMIT_TEMPERATURE:
            ...
            logger.info("Fixed temperature %r applied for model %r", profile.fixed_temperature, getattr(profile, "model", None))
        else:
        ...
        overrides = params.get("request_overrides")
        if overrides:
            # R-060: cache_prompt is llama.cpp-specific — route to extra_body
            _llama_keys = {"cache_prompt"}
            for k, v in overrides.items():
                if k == "extra_body" and isinstance(v, dict):
                    extra_body.update(v)
                elif k in _llama_keys:
                    extra_body[k] = v
                else:
                    api_kwargs[k] = v
