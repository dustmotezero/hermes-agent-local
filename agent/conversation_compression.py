    ...
    try:
        ...
        if len(compressed) == _pre_msg_count:
            logger.warning(
                ...
                "rough_tokens=~%s - compress() ran but returned the input "
                "unchanged. Retry will use the ORIGINAL size and may fail "
                "identically if already over the context limit.",
                agent.session_id or "none", _pre_msg_count,
                f"{_compressed_est:,}",
            )
        else:
        ...
        _emit_compression_attempt_telemetry(
            ...
            failure_class=(
                ...
                else ("compression_noop" if len(compressed) == _pre_msg_count else None)
            ),
        )
