...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    if not _soul_loaded and system_message is None:
        # Fallback to hardcoded identity. Skipped when the caller supplied
        # their own system_message (e.g. squad-router builds a real,
        # per-agent SOUL.md-derived persona itself) -- such callers already
        # have a complete identity landing in the context tier below, and
        # do not need generic Hermes-Agent boilerplate stacked ahead of it.
